// Generated by Bisonc++ V4.01.00 on Wed, 08 Aug 2012 03:02:04 +0430
...
PI__ const s_productionInfo[] = 
{
     ...
     {553, 1}, // 1: start ->  init
     {553, 1}, // 2: start ->  server_param
     {553, 1}, // 3: start ->  player_param
     {553, 1}, // 4: start ->  player_type
     {553, 1}, // 5: start ->  sense_body
     {553, 1}, // 6: start ->  see
     {560, 1}, // 7: bool (_0) ->  _0
     {560, 1}, // 8: bool (_1) ->  _1
     {561, 1}, // 9: int (INTEGER) ->  INTEGER
     {561, 1}, // 10: int ->  bool
     {562, 1}, // 11: double (DOUBLE) ->  DOUBLE
     {562, 1}, // 12: double ->  int
     {563, 1}, // 13: string (STRING) ->  STRING
     {554, 6}, // 14: init (LP) ->  LP INIT side double play_mode RP
     {565, 1}, // 15: play_mode (PM_BKO) ->  PM_BKO
     {565, 1}, // 16: play_mode (PM_PO) ->  PM_PO
     {565, 1}, // 17: play_mode (PM_TO) ->  PM_TO
     {565, 1}, // 18: play_mode (PM_KOL) ->  PM_KOL
     {565, 1}, // 19: play_mode (PM_KOR) ->  PM_KOR
     {565, 1}, // 20: play_mode (PM_KIL) ->  PM_KIL
     {565, 1}, // 21: play_mode (PM_KIR) ->  PM_KIR
     {565, 1}, // 22: play_mode (PM_FKL) ->  PM_FKL
     {565, 1}, // 23: play_mode (PM_FKR) ->  PM_FKR
     {565, 1}, // 24: play_mode (PM_CKL) ->  PM_CKL
     {565, 1}, // 25: play_mode (PM_CKR) ->  PM_CKR
     {565, 1}, // 26: play_mode (PM_GKL) ->  PM_GKL
     {565, 1}, // 27: play_mode (PM_GKR) ->  PM_GKR
     {565, 1}, // 28: play_mode (PM_GOALL) ->  PM_GOALL
     {565, 1}, // 29: play_mode (PM_GOALR) ->  PM_GOALR
     {565, 1}, // 30: play_mode (PM_DB) ->  PM_DB
     {565, 1}, // 31: play_mode (PM_OL) ->  PM_OL
     {565, 1}, // 32: play_mode (PM_OR) ->  PM_OR
     {555, 191}, // 33: server_param (LP) ->  LP SERVER acd am bdr bp bam bd br bs bsm bsa bw cbc cp cal caw cm cadw cdefw cdelw ciw cmd cmpc cmw crw cws co cop cwr cw cr das dpr dbt ed edt ein eit ei em eht es fko fc fdp fe fkf fsp fwp fl fr glc gld gldir glf glfn glv gl gow gw gmm gg ht hd hi hm im k kl kld kldir lkf klfn klg ks kw kow kpr kr krfl krfr km lf ldf lt mbtp maxda mdp mgk mtp maxm maxna maxnm maxp minda mindp minm minna minnm minp neh nnh oaas okm op och pamk pbsw pcmp pdx pmet pmgdx pnk prw prwa psw ptw pso pam pd pr ps psm psmm pw ptb ptd p pfl pfr prof pgk qs qsl rm rd rdt ri rmin rs rcp sccm scms sms sm ss svs sbs sdr sims sdf sotflt sotfrt sc sim smax sgl sgr sbv smics smod so sso tbd tc td te tpr trf tw tan tls trs tlc tld tldir tlf tlfn tl uo v va vd wa wd wf wn wr wra RP
     {566, 4}, // 34: acd (LP) ->  LP ACD double RP
     {567, 4}, // 35: am (LP) ->  LP AM bool RP
     {568, 4}, // 36: bdr (LP) ->  LP BDR double RP
     {569, 4}, // 37: bp (LP) ->  LP BP double RP
     {570, 4}, // 38: bam (LP) ->  LP BAM double RP
     {571, 4}, // 39: bd (LP) ->  LP BD double RP
     {572, 4}, // 40: br (LP) ->  LP BR double RP
     {573, 4}, // 41: bs (LP) ->  LP BS double RP
     {574, 4}, // 42: bsm (LP) ->  LP BSM double RP
     {575, 4}, // 43: bsa (LP) ->  LP BSA double RP
     {576, 4}, // 44: bw (LP) ->  LP BW double RP
     {577, 4}, // 45: cbc (LP) ->  LP CBC int RP
     {578, 4}, // 46: cp (LP) ->  LP CP double RP
     {579, 4}, // 47: cal (LP) ->  LP CAL double RP
     {580, 4}, // 48: caw (LP) ->  LP CAW double RP
     {581, 4}, // 49: cm (LP) ->  LP CM double RP
     {582, 4}, // 50: cadw (LP) ->  LP CADW int RP
     {583, 4}, // 51: cdefw (LP) ->  LP CDEFW int RP
     {584, 4}, // 52: cdelw (LP) ->  LP CDELW int RP
     {585, 4}, // 53: ciw (LP) ->  LP CIW int RP
     {586, 4}, // 54: cmd (LP) ->  LP CMD int RP
     {587, 4}, // 55: cmpc (LP) ->  LP CMPC int RP
     {588, 4}, // 56: cmw (LP) ->  LP CMW int RP
     {589, 4}, // 57: crw (LP) ->  LP CRW int RP
     {590, 4}, // 58: cws (LP) ->  LP CWS int RP
     {591, 4}, // 59: co (LP) ->  LP CO double RP
     {592, 4}, // 60: cop (LP) ->  LP COP int RP
     {593, 4}, // 61: cwr (LP) ->  LP CWR double RP
     {594, 4}, // 62: cw (LP) ->  LP CW int RP
     {595, 4}, // 63: cr (LP) ->  LP CR double RP
     {596, 4}, // 64: das (LP) ->  LP DAS double RP
     {597, 4}, // 65: dpr (LP) ->  LP DPR double RP
     {754, 4}, // 66: pt_dpr (LP) ->  LP DPR double RP
     {598, 4}, // 67: dbt (LP) ->  LP DBT int RP
     {599, 4}, // 68: ed (LP) ->  LP ED double RP
     {600, 4}, // 69: edt (LP) ->  LP EDT double RP
     {601, 4}, // 70: ein (LP) ->  LP EIN double RP
     {602, 4}, // 71: eit (LP) ->  LP EIT double RP
     {603, 4}, // 72: ei (LP) ->  LP EI double RP
     {604, 4}, // 73: em (LP) ->  LP EM double RP
     {755, 4}, // 74: pt_emx (LP) ->  LP EMAX double RP
     {756, 4}, // 75: pt_emi (LP) ->  LP EM double RP
     {605, 4}, // 76: eht (LP) ->  LP EHT int RP
     {606, 4}, // 77: es (LP) ->  LP ES double RP
     {757, 4}, // 78: pt_es (LP) ->  LP ES double RP
     {607, 4}, // 79: fko (LP) ->  LP FKO double RP
     {608, 4}, // 80: fc (LP) ->  LP FC int RP
     {609, 4}, // 81: fdp (LP) ->  LP FDP double RP
     {758, 4}, // 82: pt_fdp (LP) ->  LP FDP double RP
     {610, 4}, // 83: fe (LP) ->  LP FE double RP
     {611, 4}, // 84: fkf (LP) ->  LP FKF double RP
     {612, 4}, // 85: fsp (LP) ->  LP FSP double RP
     {613, 4}, // 86: fwp (LP) ->  LP FWP double RP
     {614, 4}, // 87: fl (LP) ->  LP FL int RP
     {615, 4}, // 88: fr (LP) ->  LP FR int RP
     {616, 4}, // 89: glc (LP) ->  LP GLC int RP
     {617, 4}, // 90: gld (LP) ->  LP GLD int RP
     {618, 4}, // 91: gldir (LP) ->  LP GLDIR string RP
     {619, 4}, // 92: glf (LP) ->  LP GLF double RP
     {620, 4}, // 93: glfn (LP) ->  LP GLFN string RP
     {621, 4}, // 94: glv (LP) ->  LP GLV int RP
     {622, 4}, // 95: gl (LP) ->  LP GL double RP
     {623, 4}, // 96: gow (LP) ->  LP GOW double RP
     {624, 4}, // 97: gw (LP) ->  LP GW double RP
     {625, 4}, // 98: gmm (LP) ->  LP GMM int RP
     {626, 4}, // 99: gg (LP) ->  LP GG double RP
     {627, 4}, // 100: ht (LP) ->  LP HT int RP
     {628, 4}, // 101: hd (LP) ->  LP HD double RP
     {629, 4}, // 102: hi (LP) ->  LP HI double RP
     {630, 4}, // 103: hm (LP) ->  LP HM double RP
     {631, 4}, // 104: im (LP) ->  LP IM double RP
     {759, 4}, // 105: pt_im (LP) ->  LP IM double RP
     {632, 4}, // 106: k (LP) ->  LP K double RP
     {633, 4}, // 107: kl (LP) ->  LP KL double RP
     {634, 4}, // 108: kld (LP) ->  LP KLD double RP
     {635, 4}, // 109: kldir (LP) ->  LP KLDIR string RP
     {636, 4}, // 110: lkf (LP) ->  LP LKF double RP
     {637, 4}, // 111: klfn (LP) ->  LP KLFN string RP
     {638, 4}, // 112: klg (LP) ->  LP KLG double RP
     {639, 4}, // 113: ks (LP) ->  LP KS int RP
     {640, 4}, // 114: kw (LP) ->  LP KW double RP
     {641, 4}, // 115: kow (LP) ->  LP KOW int RP
     {642, 4}, // 116: kpr (LP) ->  LP KPR double RP
     {760, 4}, // 117: pt_kpr (LP) ->  LP KPR double RP
     {643, 4}, // 118: kr (LP) ->  LP KR double RP
     {761, 4}, // 119: pt_kr (LP) ->  LP KR double RP
     {644, 4}, // 120: krfl (LP) ->  LP KRFL double RP
     {645, 4}, // 121: krfr (LP) ->  LP KRFR double RP
     {646, 4}, // 122: km (LP) ->  LP KM double RP
     {762, 4}, // 123: pt_km (LP) ->  LP KM double RP
     {647, 4}, // 124: lf (LP) ->  LP LF string RP
     {648, 4}, // 125: ldf (LP) ->  LP LDF string RP
     {649, 4}, // 126: lt (LP) ->  LP LT int RP
     {650, 4}, // 127: mbtp (LP) ->  LP MBTP double RP
     {651, 4}, // 128: maxda (LP) ->  LP MAXDA double RP
     {652, 4}, // 129: mdp (LP) ->  LP MDP double RP
     {653, 4}, // 130: mgk (LP) ->  LP MGK int RP
     {654, 4}, // 131: mtp (LP) ->  LP MTP double RP
     {655, 4}, // 132: maxm (LP) ->  LP MAXM double RP
     {656, 4}, // 133: maxna (LP) ->  LP MAXNA double RP
     {657, 4}, // 134: maxnm (LP) ->  LP MAXNM double RP
     {658, 4}, // 135: maxp (LP) ->  LP MAXP double RP
     {659, 4}, // 136: minda (LP) ->  LP MINDA double RP
     {660, 4}, // 137: mindp (LP) ->  LP MINDP double RP
     {661, 4}, // 138: minm (LP) ->  LP MINM double RP
     {662, 4}, // 139: minna (LP) ->  LP MINNA double RP
     {663, 4}, // 140: minnm (LP) ->  LP MINNM double RP
     {664, 4}, // 141: minp (LP) ->  LP MINP double RP
     {665, 4}, // 142: neh (LP) ->  LP NEH int RP
     {666, 4}, // 143: nnh (LP) ->  LP NNH int RP
     {667, 4}, // 144: oaas (LP) ->  LP OAAS double RP
     {668, 4}, // 145: okm (LP) ->  LP OKM double RP
     {669, 4}, // 146: op (LP) ->  LP OP int RP
     {670, 4}, // 147: och (LP) ->  LP OCH double RP
     {671, 4}, // 148: pamk (LP) ->  LP PAMK double RP
     {672, 4}, // 149: pbsw (LP) ->  LP PBSW double RP
     {673, 4}, // 150: pcmp (LP) ->  LP PCMP double RP
     {674, 4}, // 151: pdx (LP) ->  LP PDX double RP
     {675, 4}, // 152: pmet (LP) ->  LP PMET int RP
     {676, 4}, // 153: pmgdx (LP) ->  LP PMGDX int RP
     {677, 4}, // 154: pnk (LP) ->  LP PNK int RP
     {678, 4}, // 155: prw (LP) ->  LP PRW double RP
     {679, 4}, // 156: prwa (LP) ->  LP PRWA int RP
     {680, 4}, // 157: psw (LP) ->  LP PSW int RP
     {681, 4}, // 158: ptw (LP) ->  LP PTW int RP
     {682, 4}, // 159: pso (LP) ->  LP PSO double RP
     {683, 4}, // 160: pam (LP) ->  LP PAM double RP
     {684, 4}, // 161: pd (LP) ->  LP PD double RP
     {763, 4}, // 162: pt_pd (LP) ->  LP PD double RP
     {685, 4}, // 163: pr (LP) ->  LP PR double RP
     {686, 4}, // 164: ps (LP) ->  LP PS double RP
     {764, 4}, // 165: pt_ps (LP) ->  LP PS double RP
     {687, 4}, // 166: psm (LP) ->  LP PSM double RP
     {765, 4}, // 167: pt_psm (LP) ->  LP PSM double RP
     {688, 4}, // 168: psmm (LP) ->  LP PSMM double RP
     {689, 4}, // 169: pw (LP) ->  LP PW double RP
     {690, 4}, // 170: ptb (LP) ->  LP PTB int RP
     {691, 4}, // 171: ptd (LP) ->  LP PTD int RP
     {692, 4}, // 172: p (LP) ->  LP P int RP
     {693, 4}, // 173: pfl (LP) ->  LP PFL double RP
     {694, 4}, // 174: pfr (LP) ->  LP PFR double RP
     {695, 4}, // 175: prof (LP) ->  LP PROF double RP
     {696, 4}, // 176: pgk (LP) ->  LP PGK double RP
     {697, 4}, // 177: qs (LP) ->  LP QS double RP
     {698, 4}, // 178: qsl (LP) ->  LP QSL double RP
     {699, 4}, // 179: rm (LP) ->  LP RM double RP
     {700, 4}, // 180: rd (LP) ->  LP RD double RP
     {701, 4}, // 181: rdt (LP) ->  LP RDT double RP
     {702, 4}, // 182: ri (LP) ->  LP RI double RP
     {703, 4}, // 183: rmin (LP) ->  LP RMIN double RP
     {704, 4}, // 184: rs (LP) ->  LP RS int RP
     {705, 4}, // 185: rcp (LP) ->  LP RCP double RP
     {706, 4}, // 186: sccm (LP) ->  LP SCCM int RP
     {707, 4}, // 187: scms (LP) ->  LP SCMS int RP
     {708, 4}, // 188: sms (LP) ->  LP SMS int RP
     {709, 4}, // 189: sm (LP) ->  LP SM double RP
     {710, 4}, // 190: ss (LP) ->  LP SS int RP
     {711, 4}, // 191: svs (LP) ->  LP SVS int RP
     {712, 4}, // 192: sbs (LP) ->  LP SBS int RP
     {713, 4}, // 193: sdr (LP) ->  LP SDR double RP
     {714, 4}, // 194: sims (LP) ->  LP SIMS int RP
     {715, 4}, // 195: sdf (LP) ->  LP SDF int RP
     {716, 4}, // 196: sotflt (LP) ->  LP SOTFLT double RP
     {717, 4}, // 197: sotfrt (LP) ->  LP SOTFRT double RP
     {718, 4}, // 198: sc (LP) ->  LP SC double RP
     {719, 4}, // 199: sim (LP) ->  LP SIM double RP
     {766, 4}, // 200: pt_sim (LP) ->  LP SIM double RP
     {720, 4}, // 201: smax (LP) ->  LP SMAX double RP
     {721, 4}, // 202: sgl (LP) ->  LP SGL int RP
     {722, 4}, // 203: sgr (LP) ->  LP SGR int RP
     {723, 4}, // 204: sbv (LP) ->  LP SBV double RP
     {724, 4}, // 205: smics (LP) ->  LP SMICS double RP
     {725, 4}, // 206: smod (LP) ->  LP SMOD double RP
     {726, 4}, // 207: so (LP) ->  LP SO double RP
     {727, 4}, // 208: sso (LP) ->  LP SSO double RP
     {728, 4}, // 209: tbd (LP) ->  LP TBD double RP
     {729, 4}, // 210: tc (LP) ->  LP TC double RP
     {730, 4}, // 211: td (LP) ->  LP TD double RP
     {731, 4}, // 212: te (LP) ->  LP TE double RP
     {732, 4}, // 213: tpr (LP) ->  LP TPR double RP
     {733, 4}, // 214: trf (LP) ->  LP TRF double RP
     {734, 4}, // 215: tw (LP) ->  LP TW double RP
     {735, 4}, // 216: tan (LP) ->  LP TAN double RP
     {736, 4}, // 217: tls (LP) ->  LP TLS string RP
     {737, 4}, // 218: trs (LP) ->  LP TRS string RP
     {738, 4}, // 219: tlc (LP) ->  LP TLC int RP
     {739, 4}, // 220: tld (LP) ->  LP TLD int RP
     {740, 4}, // 221: tldir (LP) ->  LP TLDIR string RP
     {741, 4}, // 222: tlf (LP) ->  LP TLF int RP
     {742, 4}, // 223: tlfn (LP) ->  LP TLFN string RP
     {743, 4}, // 224: tl (LP) ->  LP TL double RP
     {744, 4}, // 225: uo (LP) ->  LP UO double RP
     {745, 4}, // 226: v (LP) ->  LP V bool RP
     {746, 4}, // 227: va (LP) ->  LP VA double RP
     {747, 4}, // 228: vd (LP) ->  LP VD double RP
     {748, 4}, // 229: wa (LP) ->  LP WA double RP
     {749, 4}, // 230: wd (LP) ->  LP WD double RP
     {750, 4}, // 231: wf (LP) ->  LP WF double RP
     {751, 4}, // 232: wn (LP) ->  LP WN double RP
     {752, 4}, // 233: wr (LP) ->  LP WR double RP
     {753, 4}, // 234: wra (LP) ->  LP WRA double RP
     {556, 32}, // 235: player_param (LP) ->  LP PP amdt calsmax calsmin dprdmax dprdmin emaxdf emindf esdmax esdmin fdpdf imdf kprdmax kprdmin kprdf kmdmax kmdmin ndprdmax ndprdmin nsimaxdf pddmax pddmin psdf psmaxdmax psmaxdmin pt ptm rans simaxdf submax RP
     {557, 18}, // 236: player_type (LP) ->  LP PTY id pt_psm pt_sim pt_pd pt_im pt_dpr pt_ps pt_km pt_kr pt_es pt_emx pt_emi pt_kpr pt_fdp pt_cals RP
     {767, 4}, // 237: amdt (LP) ->  LP AMDT double RP
     {768, 4}, // 238: calsmax (LP) ->  LP CALSMAX double RP
     {797, 4}, // 239: pt_cals (LP) ->  LP CALS double RP
     {769, 4}, // 240: calsmin (LP) ->  LP CALSMIN double RP
     {770, 4}, // 241: dprdmax (LP) ->  LP DPRDMAX double RP
     {771, 4}, // 242: dprdmin (LP) ->  LP DPRDMIN double RP
     {772, 4}, // 243: emaxdf (LP) ->  LP EMAXDF double RP
     {773, 4}, // 244: emindf (LP) ->  LP EMINDF double RP
     {774, 4}, // 245: esdmax (LP) ->  LP ESDMAX double RP
     {775, 4}, // 246: esdmin (LP) ->  LP ESDMIN double RP
     {776, 4}, // 247: fdpdf (LP) ->  LP FDPDF double RP
     {777, 4}, // 248: imdf (LP) ->  LP IMDF double RP
     {778, 4}, // 249: kprdmax (LP) ->  LP KPRDMAX double RP
     {779, 4}, // 250: kprdmin (LP) ->  LP KPRDMIN double RP
     {780, 4}, // 251: kprdf (LP) ->  LP KPRDF double RP
     {781, 4}, // 252: kmdmax (LP) ->  LP KMDMAX double RP
     {782, 4}, // 253: kmdmin (LP) ->  LP KMDMIN double RP
     {783, 4}, // 254: ndprdmax (LP) ->  LP NDPRDMAX double RP
     {784, 4}, // 255: ndprdmin (LP) ->  LP NDPRDMIN double RP
     {785, 4}, // 256: nsimaxdf (LP) ->  LP NSIMAXDF double RP
     {786, 4}, // 257: pddmax (LP) ->  LP PDDMAX double RP
     {787, 4}, // 258: pddmin (LP) ->  LP PDDMIN double RP
     {788, 4}, // 259: psdf (LP) ->  LP PSDF double RP
     {789, 4}, // 260: psmaxdmax (LP) ->  LP PSMAXDMAX double RP
     {790, 4}, // 261: psmaxdmin (LP) ->  LP PSMAXDMIN double RP
     {791, 4}, // 262: pt (LP) ->  LP PT int RP
     {792, 4}, // 263: ptm (LP) ->  LP PTM int RP
     {793, 4}, // 264: rans (LP) ->  LP RANS int RP
     {794, 4}, // 265: simaxdf (LP) ->  LP SIMAXDF double RP
     {795, 4}, // 266: submax (LP) ->  LP SUBMAX int RP
     {796, 4}, // 267: id (LP) ->  LP ID int RP
     {558, 21}, // 268: sense_body (LP) ->  LP SEN double vm st sp ha kick dash turn say tneck catch move chview arm focus tackle collision foul RP
     {798, 5}, // 269: vm (LP) ->  LP VM v_q v_w RP
     {815, 1}, // 270: v_q (VQ_H) ->  VQ_H
     {815, 1}, // 271: v_q (VQ_L) ->  VQ_L
     {816, 1}, // 272: v_w (VW_NO) ->  VW_NO
     {816, 1}, // 273: v_w (VW_NA) ->  VW_NA
     {816, 1}, // 274: v_w (VW_WI) ->  VW_WI
     {799, 6}, // 275: st (LP) ->  LP ST double double double RP
     {800, 5}, // 276: sp (LP) ->  LP SP double double RP
     {801, 4}, // 277: ha (LP) ->  LP HA int RP
     {802, 4}, // 278: kick (LP) ->  LP KICK int RP
     {803, 4}, // 279: dash (LP) ->  LP DASH int RP
     {804, 4}, // 280: turn (LP) ->  LP TURN int RP
     {805, 4}, // 281: say (LP) ->  LP SAY int RP
     {806, 4}, // 282: tneck (LP) ->  LP TNECK int RP
     {807, 4}, // 283: catch (LP) ->  LP CATCH int RP
     {808, 4}, // 284: move (LP) ->  LP MOVE int RP
     {809, 4}, // 285: chview (LP) ->  LP CHVIEW int RP
     {810, 7}, // 286: arm (LP) ->  LP ARM movable expires target count RP
     {817, 4}, // 287: movable (LP) ->  LP MOVABLE int RP
     {818, 4}, // 288: expires (LP) ->  LP EXP int RP
     {819, 5}, // 289: target (LP) ->  LP TAR int int RP
     {820, 4}, // 290: count (LP) ->  LP COUNT int RP
     {811, 5}, // 291: focus (LP) ->  LP FOCUS f_target f_count RP
     {822, 4}, // 292: f_count (LP) ->  LP COUNT int RP
     {821, 4}, // 293: f_target (LP) ->  LP TAR target_type RP
     {823, 1}, // 294: target_type ->  side
     {823, 1}, // 295: target_type (NONE) ->  NONE
     {812, 5}, // 296: tackle (LP) ->  LP TACKLE t_expires t_count RP
     {824, 4}, // 297: t_expires (LP) ->  LP EXP int RP
     {825, 4}, // 298: t_count (LP) ->  LP COUNT int RP
     {813, 4}, // 299: collision (LP) ->  LP COLL col_type RP
     {826, 1}, // 300: col_type (NONE) ->  NONE
     {826, 1}, // 301: col_type (BALL) ->  BALL
     {826, 1}, // 302: col_type (PLAYER) ->  PLAYER
     {826, 1}, // 303: col_type (POST) ->  POST
     {814, 5}, // 304: foul (LP) ->  LP FOUL charged card RP
     {827, 4}, // 305: charged (LP) ->  LP CHARGED int RP
     {828, 4}, // 306: card (LP) ->  LP CARD card_type RP
     {829, 1}, // 307: card_type (NONE) ->  NONE
     {829, 1}, // 308: card_type (YELLOW) ->  YELLOW
     {559, 5}, // 309: see (LP) ->  LP SEE int objects RP
     {830, 2}, // 310: objects ->  object objects
     {830, 1}, // 311: objects ->  object
     {831, 1}, // 312: object ->  o_ball
     {831, 1}, // 313: object ->  o_player
     {831, 1}, // 314: object ->  o_goal
     {831, 1}, // 315: object ->  o_flag
     {832, 7}, // 316: o_ball (LP) ->  LP LP _B RP double double RP
     {832, 9}, // 317: o_ball (LP) ->  LP LP _B RP double double double double RP
     {833, 13}, // 318: o_player (LP) ->  LP LP _P string int RP double double double double double double RP
     {833, 8}, // 319: o_player (LP) ->  LP LP _P string RP double double RP
     {833, 7}, // 320: o_player (LP) ->  LP LP _P RP double double RP
     {834, 8}, // 321: o_goal (LP) ->  LP LP _G side RP double double RP
     {834, 10}, // 322: o_goal (LP) ->  LP LP _G side RP double double double double RP
     {835, 7}, // 323: o_flag (LP) ->  LP LP flag_name RP double double RP
     {835, 9}, // 324: o_flag (LP) ->  LP LP flag_name RP double double double double RP
     {836, 2}, // 325: flag_name (_F) ->  _F _C
     {836, 3}, // 326: flag_name (_F) ->  _F side tb
     {836, 3}, // 327: flag_name (_F) ->  _F _C tb
     {836, 4}, // 328: flag_name (_F) ->  _F _P side tcb
     {836, 4}, // 329: flag_name (_F) ->  _F _G side tb
     {836, 3}, // 330: flag_name (_F) ->  _F lrtb _0
     {836, 4}, // 331: flag_name (_F) ->  _F tb side int
     {836, 4}, // 332: flag_name (_F) ->  _F side tb int
     {836, 2}, // 333: flag_name (_L) ->  _L lrtb
     {836, 1}, // 334: flag_name (_BB) ->  _BB
     {836, 1}, // 335: flag_name (_FF) ->  _FF
     {836, 1}, // 336: flag_name (_GG) ->  _GG
     {836, 1}, // 337: flag_name (_PP) ->  _PP
     {837, 1}, // 338: tb (_T) ->  _T
     {837, 1}, // 339: tb (_B) ->  _B
     {564, 1}, // 340: side (_L) ->  _L
     {564, 1}, // 341: side (_R) ->  _R
     {838, 1}, // 342: tcb (_T) ->  _T
     {838, 1}, // 343: tcb (_C) ->  _C
     {838, 1}, // 344: tcb (_B) ->  _B
     {839, 1}, // 345: lrtb (_L) ->  _L
     {839, 1}, // 346: lrtb (_R) ->  _R
     {839, 1}, // 347: lrtb (_T) ->  _T
     {839, 1}, // 348: lrtb (_B) ->  _B
     {840, 1}, // 349: start_$ ->  start
};
